...
export default function IntentPage() {
  ...
  return (
    <main className="ys-form-page">
      ...
      <p className="ys-form-intro">Describe the experiences that matter to you. Your words are sent to the configured intent service and returned with transparent experience tags.</p>
      ...
      <div className="ys-intent-preview" aria-live="polite">
        <strong>Recognized experience tags</strong>
        <p>{normalized.length ? normalized.join(' · ') : 'Tags will appear after your intent is saved.'}</p>
      </div>
      <div className="ys-form-actions">
        <span>{profile.experienceIntent.length}/500 characters · {status === 'saved' ? 'Saved to your traveller account' : status === 'error' ? 'Could not save intent to the server' : 'Save your intent before continuing'}</span>
        <div>
